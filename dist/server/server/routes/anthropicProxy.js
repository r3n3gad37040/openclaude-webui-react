...
function createAnthropicToOpenAITransform(model) {
...
router.all('*', async (c) => {
    ...
    if (!upstreamRes.ok) {
        ...
        return c.json({ error: { message: `Anthropic API error ${upstreamRes.status}: ${errText.slice(0, 300)}`, type: 'api_error' } }, upstreamRes.status);
    }
