...
function firecrawlError(status, detail) {
    return new Response(JSON.stringify({ success: false, error: detail }), { status, headers: { 'Content-Type': 'application/json' } });
}
// ─── Scrape ────────────────────────────────────────────────────────────────
...
router.post('/scrape', async (c) => {
    const key = getApiKey();
    if (!key)
        return firecrawlError(401, 'Firecrawl API key not configured');
    const body = await parseJson(c);
    if (!body.url)
        return firecrawlError(400, 'url is required');
...
router.post('/search', async (c) => {
    ...
    const body = await parseJson(c);
    if (!body.query)
        return firecrawlError(400, 'query is required');
...
router.post('/search/:jobId/feedback', async (c) => {
    ...
    if (typeof body.rating !== 'number')
        return firecrawlError(400, 'rating is required');
...
router.get('/status/:id', async (c) => {
    ...
    if (res.status === 404) {
        ...
        return c.json(searchData, searchRes.status);
    }
    const data = await res.json().catch(() => ({ success: false, error: 'Invalid JSON from Firecrawl' }));
    return c.json(data, res.status);
});
