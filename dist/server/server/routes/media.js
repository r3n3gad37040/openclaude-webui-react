...
function streamFile(filePath, contentType, totalSize, range) {
    const headers = {
    ...
    return new Response(Readable.toWeb(node), { headers });
}
...
router.get('/media/serve', async (c) => {
    ...
    if (!safePath)
        return c.json({ error: 'Access denied — path not in allowed directories' }, 403);
