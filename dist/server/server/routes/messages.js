...
async function fetchMemoryContext() {
    if (CLAUDE_MEM_DISABLED)
        return '';
    try {
        const res = await fetch(`${CLAUDE_MEM_URL}/api/context/inject?projects=${CLAUDE_MEM_PROJECTS}`, { signal: AbortSignal.timeout(3000) });
        if (!res.ok)
            return '';
...
async function initMemSession(contentSessionId, firstPrompt) {
    if (CLAUDE_MEM_DISABLED)
        return;
    try {
...
async function saveMemSummary(contentSessionId, request, completed) {
    if (CLAUDE_MEM_DISABLED)
        return;
    try {
...
function buildConversationPrompt(history, content, memCtx, contextWindow = 128000) {
    ...
    for (let i = history.length - 1; i >= 0; i--) {
        ...
        if (used + t > Math.max(0, historyBudget))
            break;
        parts.unshift(entry);
        used += t;
    }
...
async function generateMusicInline(stream, sessionId, provider, model, prompt, userContent) {
    ...
    const cfg = MUSIC_MODEL_DEFAULTS[model] ?? { format: 'mp3' };
    ...
    // actually accepts.
    const queueBody = { model, prompt: prompt.slice(0, 500) };
    if (cfg.duration_seconds !== undefined)
        queueBody['duration_seconds'] = cfg.duration_seconds;
    if (cfg.reusePromptAsLyrics)
        queueBody['lyrics_prompt'] = prompt.slice(0, 4000);
    if (cfg.extra)
        Object.assign(queueBody, cfg.extra);
    ...
    for (let i = 0; i < 60; i++) {
        ...
        if (ct.startsWith('audio/')) {
            ...
            const ext = ct.includes('flac') ? 'flac' :
                ct.includes('wav') ? 'wav' :
                    ct.includes('ogg') ? 'ogg' :
...
async function generateAudioInline(stream, sessionId, provider, model, prompt, userContent) {
...
function mediaSSE(c, sessionId, modelId, prompt, userContent) {
    return streamSSE(c, async (stream) => {
        ...
        try {
            ...
            if (url) {
                ...
                const mediaType = ['mp4', 'webm', 'ogg', 'mov', 'avi', 'mkv'].includes(ext) ? 'video' : 'image';
...
function sseStream(c, sessionId, modelId, prompt, runnerOptions = {}) {
    return streamSSE(c, async (stream) => {
        ...
        const buildExtra = () => {
            ...
            if (usage) {
                ...
                if (costs) {
                    extra['estimated_cost'] = parseFloat(((usage.input_tokens * costs.input_per_m) / 1000000 +
                        (usage.output_tokens * costs.output_per_m) / 1000000).toFixed(6));
                }
            }
...
router.post('/:id/messages', async (c) => {
    ...
    const contextWindow = modelEntry?.context_window ?? 128000;
