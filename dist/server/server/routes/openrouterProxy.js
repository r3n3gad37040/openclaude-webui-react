...
function isToolUseErrorText(payload) {
    ...
    try {
        ...
        if (parsed['error']) {
            ...
            if (TOOL_USE_KEYWORDS.some((kw) => msg.includes(kw)))
                return true;
        }
 ...
 */
async function handleStreamingResponse(upstreamRes, bodyText, doFetch) {
    if (!upstreamRes.body)
        return upstreamRes;
...
router.all('*', async (c) => {
    ...
    if (bodyText && relPath.includes('/chat/completions')) {
        try {
            const parsed = JSON.parse(bodyText);
            const actualModel = parsed['model'] ?? 'openrouter-model';
    ...
    const isStreamingChatReq = bodyText !== undefined &&
        relPath.includes('/chat/completions') &&
        (() => {
            try {
                return JSON.parse(bodyText)['stream'] === true;
            }
            catch {
                return false;
            }
    ...
    if (upstreamRes.ok && isImageRequest) {
        try {
            const respJson = await upstreamRes.json();
