...
function createGroqCompoundTransform() {
...
async function sseToJson(sseStream, model) {
...
export function createGenericProxy(opts) {
    ...
    router.all('*', async (c) => {
        ...
        if (bodyText && relPath.includes('/chat/completions')) {
            try {
                ...
                if (Array.isArray(messages)) {
                    ...
                    if (modified)
                        patchedBody = { ...patchedBody, messages };
                }
        ...
        try {
            upstreamRes = await fetch(targetUrl, {
                ...
                body: bodyText,
                signal: AbortSignal.timeout(600000), // 10 min — Deepseek can be very slow
            });
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            process.stderr.write(`[proxy:${opts.providerName}] upstream fetch failed: ${msg}\n`);
            return c.json({ error: { message: `Upstream ${opts.providerName} request failed: ${msg}`, type: 'upstream_error' } }, 502);
        }
