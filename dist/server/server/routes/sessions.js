...
import { listSessions, getSession, createSession, deleteSession, renameSession, searchSessions, exportSession, deleteMessage, truncateMessagesFrom, updateSessionMeta, importSession, } from '../services/session.js';
...
router.post('/:id/delete-message', async (c) => {
    ...
    if (body.index === undefined)
        return c.json({ error: 'Message index is required' }, 400);
...
router.post('/:id/truncate', async (c) => {
    const body = await c.req.json().catch(() => ({}));
    if (body.from_index === undefined)
        return c.json({ error: 'from_index is required' }, 400);
    const ok = await truncateMessagesFrom(c.req.param('id'), body.from_index);
    if (!ok)
        return c.json({ error: 'Truncate failed' }, 400);
    return c.json({ status: 'ok' });
});
router.post('/:id/meta', async (c) => {
    ...
    const session = await updateSessionMeta(c.req.param('id'), {
        ...
        ...(('temperature_preset' in body) ? { temperature_preset: body.temperature_preset } : {}),
    });
    if (!session)
        return c.json({ error: 'Session not found' }, 404);
