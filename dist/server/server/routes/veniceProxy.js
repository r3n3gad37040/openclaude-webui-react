...
const MEDIA_CACHE_TTL = 90000;
function pruneMediaCache() {
    const now = Date.now();
    for (const [k, v] of mediaCache)
        if (now - v.ts > MEDIA_CACHE_TTL)
            mediaCache.delete(k);
}
...
function makeFakeSSE(model, content) {
    const id = `chatcmpl-media-${Date.now()}`;
    const c = (delta) => `data: ${JSON.stringify({ id, object: 'chat.completion.chunk', model, choices: [{ index: 0, delta, finish_reason: null }] })}\n\n`;
    return (c({ role: 'assistant', content: '' }) +
        c({ content }) +
        `data: ${JSON.stringify({ id, object: 'chat.completion.chunk', model, choices: [{ index: 0, delta: {}, finish_reason: 'stop' }] })}\n\n` +
        'data: [DONE]\n\n');
}
function makeFakeJSON(model, content) {
    return {
...
async function generateVeniceVideo(authHeader, model, prompt, opts) {
    ...
    if (!queueId)
        throw new Error('Venice video queue returned no queue_id');
...
async function generateVeniceImage(authHeader, model, prompt) {
    ...
    if (!b64)
        throw new Error('Venice image API returned no image data');
...
router.all('*', async (c) => {
    ...
    if (c.req.method !== 'GET' && c.req.method !== 'HEAD') {
        ...
        if (!bounded.ok)
            return c.json({ error: bounded.reason }, bounded.status);
    ...
    if (bodyText && relPath.includes('/chat/completions')) {
        try {
            const parsed = JSON.parse(bodyText);
            const actualModel = parsed['model'] ?? 'venice-model';
            const messages = parsed['messages'];
    ...
    const doFetch = (body) => fetch(targetUrl, { method: c.req.method, headers: forwardHeaders, body });
    let upstreamRes;
    try {
