...
function mediaCacheKey(model, messages) {
...
async function generateImage(authHeader, model, prompt) {
    ...
    if (!url)
        throw new Error('xAI image API returned no URL');
    return url;
}
async function generateVideo(authHeader, model, prompt) {
    ...
    if (!requestId)
        throw new Error('xAI video API returned no request_id');
