...
function sniffExt(base64) {
    ...
    if (head.startsWith('iVBORw'))
        return 'png'; // PNG
    if (head.startsWith('/9j/') || head.startsWith('/9k'))
        return 'jpg'; // JPEG
    if (head.startsWith('UklGR'))
        return 'webp'; // WebP (RIFF)
    if (head.startsWith('R0lGOD'))
        return 'gif'; // GIF
    if (head.startsWith('AAAAI') || head.startsWith('AAAAH'))
        return 'mp4';
    return 'bin';
}
// Saves a base64-encoded media blob to STATE_DIR/media and returns an
// /api/media/serve URL the frontend can fetch directly.
export function saveBase64Media(base64, ext) {
...
export function saveBytesMedia(bytes, ext) {
