...
export const PROXY_MAP: Record<string, string> = {
  anthropic: 'http://localhost:8789/anthropic-proxy',
...
export function setProviderApiKey(provider: string, apiKey: string): void {
  ...
  // Also persist to ~/.env so keys are available to all tools/processes
  const envKey = PROVIDER_MAP[provider]?.env_key ?? TOOL_MAP[provider]?.env_key
  if (envKey) {
    const env = readEnvFile()
    env[envKey] = apiKey
    writeEnvFile(env)
